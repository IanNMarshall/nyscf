...
def read_county_shp():
	...
	#df['FIPS'] = pd.to_numeric(df['FIPS']) #want to keep FIPS as a string since that's how DB is set up
	return df
...
def gen_plot(fig):
	py.plot(fig, filename='county_A00188_data.html')


if __name__ == '__main__':
	df = read_county_shp()
	df3 = merge_dfs(df, 1)
	fig = set_plot_data(df3, '36001')
	gen_plot(fig)
	#with open('dump.txt', 'w') as outfile:
		#outfile.write(str(fig))
